...
The following tutorial illustrates how to use PAGODA routines (implemented in the scde package) using single cell data from [_Pollen et al._](www.ncbi.nlm.nih.gov/pubmed/25086649).
...
```{r models,eval=FALSE,hide=TRUE,warning=FALSE,message=FALSE}
n.cores <- 1 # number of parallel cores to use
...
For pre-defined gene sets we'll use GO annotations. To do so an environment mapping GO terms to the set of genes contained in it has been pre-computed using the following code.
```{r goenv, eval=FALSE}
...
go.env.subset <- list2env(go.env[1:200]); # convert a subset to an environment
go.env <- list2env(go.env); # convert to an environment

devtools::use_data(go.env, go.env.subset)  # save for later
```

```{r, results = 'hide'}
data(go.env.subset)
go.env <- go.env.subset
```

Calculate weighted PC magnitudes of GO gene sets for subset of GO annotations to save on computing time:
